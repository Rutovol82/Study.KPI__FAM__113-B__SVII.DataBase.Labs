...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'dpa_level') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'terrtype') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'edu_status') THEN
...
CREATE TABLE IF NOT EXISTS zno_odata_records
(
    -- Record id ('OutID')
